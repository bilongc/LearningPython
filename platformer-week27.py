...
class Platform(pygame.sprite.Sprite):
    ...
    def __str__(self):
        return f'x = {self.rect.x}, y = {self.rect.y}, name={self.name}'
...
def get_new_platform(near):
        ...
        if mid > WIDTH / 2:
            new_x = random.randint(near.rect.left - new_ratio * 48 - 96, near.rect.left)
        else:
            new_x = random.randint(near.rect.right, near.rect.right + 96)
        new_y = random.randint(near.rect.top - 100-48, near.rect.top-48)
        return Platform(new_x, new_y, new_ratio)
